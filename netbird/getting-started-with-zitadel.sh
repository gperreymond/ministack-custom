...
NETBIRD_DOMAIN="netbird.docker.localhost"
NETBIRD_IP="10.1.0.77"
...
initEnvironment() {
  ...
  if [[ $ZITADEL_DATABASE == "cockroach" ]]; then
      ...
      renderPostgresEnv > $destinaton/zdb.env
  fi

  echo Rendering initial files...
  renderDockerCompose > $destinaton/docker-compose.yml
  renderCaddyfile > $destinaton/Caddyfile
  renderZitadelEnv > $destinaton/zitadel.env
  echo "" > $destinaton/dashboard.env
  echo "" > $destinaton/turnserver.conf
  echo "" > $destinaton/management.json
  echo "" > $destinaton/relay.env

  mkdir -p $destinaton/machinekey
  chmod 777 $destinaton/machinekey
  cd $destinaton
...
services:
  ...
  caddy:
    ...
    labels:
      ...
      - 'traefik.http.routers.netbird.entrypoints=web'
    networks:
      kestra_ministack:
        ipv4_address: '10.1.0.77'
    volumes:
      - caddy_data:/data
  ...
  # Management
  management:
    image: netbirdio/management:0.36.3
    restart: unless-stopped
    extra_hosts:
      - "${NETBIRD_DOMAIN}:${NETBIRD_IP}"
    networks:
      kestra_ministack: {}
    volumes:
  ...
  coturn:
    image: coturn/coturn:edge-debian
    restart: unless-stopped
...
renderDockerComposeCockroachDB() {
  ...
  zdb:
    ...
    volumes:
      - zdb_data:/cockroach/cockroach-data
      - zdb_certs:/cockroach/certs
...
renderDockerComposePostgres() {
  ...
  zdb:
    ...
    env_file:
      - ./zdb.env
    volumes:
      - zdb_data:/var/lib/postgresql/data:rw
    healthcheck:
